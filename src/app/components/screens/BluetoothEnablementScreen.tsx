import { motion } from "motion/react";
import { Button } from "@/app/components/ui/button";
import { Bluetooth, BluetoothOff, AlertCircle } from "lucide-react";
import { useState } from "react";

interface BluetoothEnablementScreenProps {
  onEnable: () => void;
  onSkip: () => void;
}

export function BluetoothEnablementScreen({ onEnable, onSkip }: BluetoothEnablementScreenProps) {
  const [isEnabling, setIsEnabling] = useState(false);

  const handleEnable = () => {
    setIsEnabling(true);
    // Simulate Bluetooth enabling
    setTimeout(() => {
      onEnable();
    }, 1500);
  };

  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-between p-6 pb-8">
      {/* Main content */}
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-[400px]">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, ease: "easeInOut" }}
          className="flex flex-col items-center gap-8 text-center w-full"
        >
          {/* Icon */}
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.5, delay: 0.1 }}
            className="relative"
          >
            <div className="w-24 h-24 rounded-full bg-[#DBEAFE] flex items-center justify-center">
              {isEnabling ? (
                <motion.div
                  animate={{ rotate: 360 }}
                  transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
                >
                  <Bluetooth className="w-12 h-12 text-[#1E40AF]" />
                </motion.div>
              ) : (
                <BluetoothOff className="w-12 h-12 text-[#6B7280]" />
              )}
            </div>
          </motion.div>

          {/* Title */}
          <motion.h1
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="text-[32px] font-bold text-[#1F2937]"
          >
            {isEnabling ? "Enabling Bluetooth..." : "Enable Bluetooth"}
          </motion.h1>

          {/* Description */}
          <motion.p
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.3 }}
            className="text-[16px] text-[#6B7280] leading-relaxed px-4"
          >
            Bluetooth is required to connect with your YOU2ME device and discover nearby connections.
          </motion.p>

          {/* Info box */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.6, delay: 0.4 }}
            className="bg-[#FEF3C7] border border-[#FCD34D] rounded-xl p-4 flex gap-3"
          >
            <AlertCircle className="w-5 h-5 text-[#F59E0B] flex-shrink-0 mt-0.5" />
            <p className="text-[14px] text-[#92400E] leading-relaxed text-left">
              Your phone may ask for Bluetooth permission. Please allow it to continue.
            </p>
          </motion.div>
        </motion.div>
      </div>

      {/* CTAs */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.5 }}
        className="w-full flex flex-col gap-3 max-w-[400px]"
      >
        <Button
          onClick={handleEnable}
          disabled={isEnabling}
          className={`
            w-full h-12 rounded-xl flex items-center justify-center gap-3
            ${
              isEnabling
                ? "bg-[#93C5FD] text-white cursor-wait"
                : "bg-[#1E40AF] hover:bg-[#1E3A8A] text-white"
            }
          `}
        >
          <Bluetooth className="w-5 h-5" />
          {isEnabling ? "Enabling..." : "Enable Bluetooth"}
        </Button>

        <Button
          onClick={onSkip}
          disabled={isEnabling}
          variant="ghost"
          className="w-full h-12 text-[#6B7280] hover:text-[#1F2937] rounded-xl"
        >
          Skip for now
        </Button>
      </motion.div>
    </div>
  );
}
